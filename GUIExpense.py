...
def Save(event=None):
    ...
    try:
        ...
        dt = days[today] + '-' + dt
        with open('D:\web\savedata2.csv','a',encoding='utf-8',newline='') as f:
...
def read_csv():
    with open('D:\web\savedata2.csv',newline='',encoding='utf-8') as b:
        fr = csv.reader(b)
        data = list(fr)
    return data
...
headerwidth = [120,150,170,60,60,60]
...
def UpdateCSV():
    with open('D:\web\savedata2.csv','w',newline='',encoding='utf-8') as f:
        fw = csv.writer(f)

        data = list(alltransaction.values())
        fw.writerows(data)
        print('Tadle was updated')
...
def DeleteRecord(event=None):
    check = messagebox.askquestion('Cofirm?','คุณต้องการลบข้อมูลใช่หรือไม่?')
    print('YES/NO:',check)
